...
class Handler:
    def __init__(self, parent, list_of_teams):
        ...
        next_power_of_2, nb_of_round, nb_of_box = self.nb_of_round(self.get_nb_of_teams())
        ...
        for i in range(next_power_of_2):
            if i < nb_of_ghosts:
                ...
                team = Team(list_of_teams[i - nb_of_ghosts].get('name'), list_of_teams[i - nb_of_ghosts].get('color'))
                self._list_to_return.append(team)
        ...
        for i in range(nb_of_box - len(self._list_to_return)):
            not_a_team = NotATeam()
            not_a_team.coordinate = (, (i//2)*2 + 1)
            self._list_to_return.append(not_a_team)
        
        i = 0
        while (len(self._list_to_return) < nb_of_box) / 2:
            if ((i//2)*2) % 2 == 0:
                self._list_to_return[i].coordinate = (i%2, (i//2)*2) # Col, Row
